...
import { useCart, useWishlist, productGetters, useFacet, facetGetters, wishlistGetters } from '@vsf-enterprise/commercetools';
...
export default {
  ...
  setup(props, context) {
    ...
    const colorAttributes = computed(() => products.value.map(product =>  product.attributes.find(attribute => attribute.name === "color")));
    const sizeAttributes = computed(() => products.value.map(product =>  product.attributes.find(attribute => attribute.name === "size")));
    ...
    watch(()=>facets.value, (newFacets)=>{
      if(!newFacets.length) return
      selectedFilters.value = newFacets.reduce((prev, curr) => ({
        ...prev,
